...
export const api = {
  ...
  lifecycleScan: (body: { awbs: string[]; code: string; remark?: string; podDataUrl?: string; bagCode?: string; location?: string; scanAt?: string }) =>
    request<{ code: string; updated: number; done: string[]; missing: string[]; locked?: string[]; duplicate?: string[]; needPod?: string[] }>('/api/v1/lifecycle/scan', { method: 'POST', body: JSON.stringify(body) }),
  ...
  bdPickup: (awb: string, body: { date?: string; time?: string; remarks?: string }) => request<{ awb: string; token: string | null; pickupDate: string }>(`/api/v1/bluedart/pickup/${awb}`, { method: 'POST', body: JSON.stringify(body) }),
  ...
  approveDeduction: (id: string | number, body: { approvedAmount?: number; remark?: string }) => request<any>(`/api/v1/deductions/${id}/approve`, { method: 'POST', body: JSON.stringify(body) }),
  rejectDeduction: (id: string | number, body: { remark?: string }) => request<any>(`/api/v1/deductions/${id}/reject`, { method: 'POST', body: JSON.stringify(body) }),
