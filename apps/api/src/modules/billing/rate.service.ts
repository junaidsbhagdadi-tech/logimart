...
export class RateService {
  ...
  /**
   * Resolve charges for a shipment: manual override > FTL flat rate > per-kg.
  ...
  async quoteForShipment(awb: string) {
    ...
    if (!breakup) {
      throw new NotFoundException(
        `No rate for ${s.originZone}->${s.destZone} (${s.serviceMode}${s.ftlVehicleType ? ' ' + s.ftlVehicleType : ''}). Add a rate card / FTL rate, or enter an agreed freight.`,
      );
    }
